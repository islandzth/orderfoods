        ...
        <div class="row mt-5">
            <div class="card w-100">
                <div class="card-header">
                    @include('general_kitchen.partials.menu', ['current' => 'boards'])
                </div>
                <div class="card-body">
                    <div id="accordion">
                        <div class="row" id="list_food_by_board">
                            @if($boards)
                                @foreach($boards as $board)
                                        @include('general_kitchen.partials.board', ['board' => $board])
                                    @endforeach
                                @endif
                        </div>
                    </div>
...
@section('scripts')
    <script>
		$(document).ready(function () {
            ...
            loadFoodByBoardGeneralKitchen({{ $boardId }});
            @endforeach
            @endif
				...
				$.ajax({
					...
					success: function (data) {
						if(data.err) {
                            ...
                            loadFoodByBoardGeneralKitchen(data.board_id);
                        }
					}
				});
			});
